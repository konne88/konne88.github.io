...
Pictures are essential to understanding proofs and definitions ([empirical evidence][STUDY]), and are thus common in traditional pen and paper mathematics. While proof assistants offer several benefits over pen and paper mathematics, proofs and definitions in proof assistants are entirely textual (i.e. lack images), and are thus often less understandable than their traditional equivalents. This blog post aims to change this unsatisfactory situation by showing how to write pictorial definitions in a proof assistant. Specifically, this blog post shows how to write commutative diagrams for category theory in Coq.
...
Yet, despite the importance of commutative diagrams, formalizations of category theory in proof assistants ([of which there are many][LIST]) do not support them. Until now!

This blog post starts by showing how to use commutative diagrams in Coq, and
then explains how they are implemented. In this blog post I won't be able to satisfactorily explain category theory, but there are good explanations out there such as [Milewski's blog][MILEWSKI].

Following is the definition of a category theoretical `Product` using a commutative diagram in Coq.
...
Note that `∘`'s arguments are flipped in comparison to the usual mathematical definition. The source code can be downloaded from [GitHub][TODO].

The definition of `Product` is standard, except the `productOk` field. The `productOk` is defined using the ASCII art representation of a commutative diagram, which is first `parse`d and then `denote`d. The labels on the diagram (e.g. `factr` and `prjR`) are just decoration, the real labels (e.g. `(factor p q)` and `projR`) are passed as arguments to the `parse` function. Can you guess why the labels have their `o`'s removed?
...
The above is standard. Next, we have to prove `productOk`. The initial goal is (to nobodies surprise):
...
To prove this goal, we first introduce the variables, and then we actually run the diagram parser and denote the result with `compute`. It's instructive not to expand any definitions particular to `prod` or the `Coq` category.
...
This goal is identical in meaning to the formula one would use instead of a commutative diagram.
...
The ultimate goal of the parsing is to turn the pictorial representation of a diagram an internal representation named `Diagram`. This internal representation is defined below.
...
The first step toward translating pictorial diagrams to a `Diagram` is to find all vertexes of the image, along with the start and end point of each arrow. The function that does this has the following type `parseImage (img:list string) : list (Z*Z) * list ((Z*Z)*(Z*Z))`. The function isn't particularly interesting and I won't explain its implementation.
...
The `parse` implementation mirrors that of `parseType`. This means that Coq can actually infer quite a bit of code (remember the three lines of code that keep `om` and `os` up to date? that's `_` now!). Note the `fun o:object => _` which corresponds to the `forall o:object, _` in the type.
...
Because the parser actually has to keep track of the passed morphisms, its recursion over the arrows `es` is more complex than `parseType`'s recursion. 
...
The recursion maintains `ms`. `ms` is the list of the morphisms collected as parameters along with the `index` of each morphisms source `s` and destination `d`. The following code keeps `ms` up to date. As in `parseType`, morphisms are silently ignored if their source or destination is not object.
...
I.e. a diagram commutes iff for any two paths `P` and `Q` with the same source `s` and destination `d`, the compositions of the morphisms along these paths are equal. Note that the Diagram is passed around implicitly using the type class mechanism.
...
The composition of the morphisms along a path is defined as:
...
8) Support image (e.g. SVG or PNG) based commutative diagrams. It might be challenging to efficiently parse such large amounts of data in Coq. This might require an extension to proof general that renders the images instead of showing its bits.
